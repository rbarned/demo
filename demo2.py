...
class Digit:
    def __init__(self, name, zOffset): #, location = (0, 0, 0), rotation = (0, 0, 0)):
        ...
        self.upDownDelay=10
        self.pauseDelay = 10
        self.scenarioInput=0
        self.scenarioOutput=0

        bpy.ops.object.text_add(enter_editmode=False, align='WORLD', location=(4.8, 0, zOffset+.04), rotation=(1.5708, 0, 0), scale=(.3, .3, .3))
        self.textC = bpy.context.active_object
        # text1.location = (4.8,0,zOffset+.04)
        self.textC.dimensions = (.5, .16, .16)
        self.textC.name = 'Text1'
        self.textC.data.body = """Carry
Pending"""
        self.textC.hide_set(False)

        bpy.ops.mesh.primitive_gear(align='WORLD', location=(0, 0, zOffset),     rotation=(0, 0, 0), change=False, number_of_teeth=10)
        self.gearM = bpy.context.active_object
        self.gearM.name = 'Gear_middle'
        bpy.ops.mesh.primitive_gear(align='WORLD', location=(2.1, 0, zOffset),     rotation=(0, 0, 0),            change=False, number_of_teeth=10)
        self.gearR = bpy.context.active_object
        self.gearR.name = 'Gear_right'
        bpy.ops.mesh.primitive_gear(align='WORLD', location=(-2.1, 0, zOffset-.25), rotation=(0, 0, 0),            change=False, number_of_teeth=10)
        self.gearL = bpy.context.active_object
        self.gearL.dimensions[2]=.9
        self.gearL.name = 'Gear_left'
        bpy.ops.mesh.primitive_gear(align='WORLD', location=(3.7, 0, zOffset-.45),   rotation=(0, 0, 0), change=False, number_of_teeth=5)
        ...
        self.cylinder.location.z=zOffset+.4
        ...
        self.cylinder2.location.z=zOffset+.4
    ...
    def newScenario(self, scenarioInput, scenarioOutput):
        self.currentFrame = 0
        self.scenarioInput = scenarioInput
        self.scenarioOutput = scenarioOutput
        self.inputDigit = self.scenarioInput
        self.outputDigit = self.scenarioOutput

    def endScenario(self):
        self.currentFrame = 0
        self.inputDigit = self.scenarioInput
        self.outputDigit = self.scenarioOutput
        # !!!!!!!! Move next line to a base class
        bpy.data.scenes["Scene"].frame_end = self.currentFrame + self.pauseDelay
    ...
    def setadd(self):
        ...
        self.gearR.rotation_euler[2]     = radians(self.outputDigit*36)
        self.gearR.keyframe_insert("rotation_euler", frame=self.currentFrame)
        self.cylinder2.rotation_euler[2] = radians(self.outputDigit*36)+radians(-18)
        ...
        self.cylinder2.rotation_euler[2] = radians(self.outputDigit*36)+radians(-18)
        self.cylinder2.keyframe_insert("rotation_euler", frame=self.currentFrame)
        if (((self.outputDigit-self.inputDigit)%10)+self.inputDigit)>9:
            self.carry = True
            print('Carry Pending')
            self.textC.keyframe_insert("location", frame=self.currentFrame)
            self.textC.hide_set(False)
            self.textC.data.body = """Carry
Pending"""
            self.currentFrame += 1
            self.textC.location[0]=4.8
            # self.textC.location[2]=self.zOffset+.04
            self.textC.keyframe_insert("location", frame=self.currentFrame)
        else:
            self.textC.hide_set(True)
            print('Carry Not Pending')
            # self.textC.location[2]=self.zOffset-.45
            self.textC.location[0]=2.1
            # self.textC.location[2]=self.zOffset+.04
            self.textC.data.body = """Carry
Pending"""
            self.currentFrame += 1
            ...
            # self.textC.location[2]=self.zOffset-.45
            self.textC.keyframe_insert("location", frame=self.currentFrame)
        self.currentFrame += self.pauseDelay
    
    def restoreinput(self):
        self.gearM.location[2]=self.zOffset
        self.gearM.keyframe_insert("location", frame=self.currentFrame)
        self.currentFrame += self.upDownDelay
        self.gearM.location[2]=self.zOffset-.45
        self.gearM.keyframe_insert("location", frame=self.currentFrame)
...
digit1 = Digit('FirstDigit', 0) 
digit2 = Digit('FirstDigit1', 2) 
digit1.printAttributes()
print (digit1.name)
digit1.newScenario(4, 0)
digit1.setadd()
digit1.restoreinput()
digit1.setadd()
digit1.restoreinput()
